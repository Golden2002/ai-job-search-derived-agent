...
```
resume_product（简历制作主基线，MCP 25 工具 + 对话式前端）
┌──────────────────────────────────────────────────────┐
│  前端 web/index.html（对话式三栏：场景树/维度卡片/汇总）  │
│  core.py + executor.py + api.py（编排 + 统一入口）      │
└──┬──────────┬──────────┬──────────┬──────────┬───────┘
   │          │          │          │          │
   v          v          v          v          v
┌────────┐┌────────┐┌────────┐┌────────────┐┌──────────┐
│job_eval││claim_gate││multi_ver││render/     ││scene_cards│
│五维评分 ││事实校验  ││三档表达  ││docx/pdf/   ││4场景11岗位│
│        ││        ││STAR量化 ││xlsx/image  ││55阶段    │
└────────┘└────────┘└────────┘│导入还原排版 ││125字段   │
                              └────────────┘└──────────┘
   │
   v
/api/chat（LLM 约束追问 + 实体抽取 + 降级）→ 导出（PDF 应用还原 CSS）
```

**对话式收集流程**：用户输入 → `/api/chat`（场景字段约束注入 LLM）→ 实体抽取填充字段 + 追问遗漏 → 卡片/汇总同步 → 生成简历 → 导出。

## 🔌 可扩展性

| 扩展点 | 方式 | 机制 |
|---|---|---|
| **分场景卡片** | `data/scene_cards.json` 追加 | 场景→子场景→阶段→字段（改 JSON 零代码，前端自动渲染） |
...
| **LLM 后端** | 注入 `chat_fn` | 规则兜底 + LLM 增强（STAR 量化 + 对话收集） |
